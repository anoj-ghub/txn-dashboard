...
- A repeatable synthetic dataset: **1,302 rows, 14 markets × 93 months, January 2019–September 2026**. Later regeneration extends through the current month. All numbers are illustrative, including a synthetic full-month value for the current month.
...
This merges by market/year/month and replaces matching records for corrections. It checks the resulting archive for all 14 markets and missing months before writing. Partial new months cannot be published. Existing months can receive targeted corrections. Renamed markets require a consistent historical update. Review the diff and commit the CSV and metadata together.
