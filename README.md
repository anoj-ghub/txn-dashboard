...
The dashboard uses larger typography and a premium blue theme, monthly transaction bars with a descriptive fitted trendline, active-plastic market-share donuts, horizontal bars for active basic, and an active-account area/trend chart. Year comparisons show a separate donut and Basic chart for every included year, newest first, with consistent market colors and a shared Basic value scale. Market comparison switches transaction bars into market stacks; market legends, ranking labels, and the market scorecard drill into a selection. Every chart follows the selected markets, timeframe, and comparison mode. The selected comparison metric uses a solid blue button and checkmark. Definitions, filtered CSV export, shareable links, and browser print styles are included.
