...
The dashboard uses larger typography and a premium blue theme, monthly transaction bars with a fitted trendline, a large active-plastic pie, grouped bars for active basic, and an active-account area/trend chart. The pie and Basic chart sit side by side. In year modes, each pie slice represents a year's snapshot and each Basic market group contains differently colored bars for the compared years. In monthly mode, pie slices and Basic bars represent months in the selected year. Matching colors connect the two charts; hover details show each period's market breakdown. Pie slices compare snapshot sizes and do not represent an additive portfolio total across time. Market labels and the scorecard drill into a selection. The selected comparison metric uses a solid blue button and checkmark. Definitions, filtered CSV export, shareable links, and browser print styles are included.
...
The transaction trendline is an ordinary least-squares fit within the observed period, not a forecast. It is omitted for fewer than two months or an incomplete selected period. Each plastic pie slice represents a year or month, with absolute values in the legend and market details on hover. Basic bars show the reporting year's top six markets by default; All markets includes every selected market.
