...
const metrics = METRICS.map((metric, index) => ({ ...metric, color: ['#006FCF', '#0891b2', '#d08a24', '#7c3aed'][index] }));
const icons = [Activity, CreditCard, WalletCards, Users];
const marketColors = ['#006FCF', '#0891b2', '#7c3aed', '#3b82f6', '#6366f1', '#06b6d4', '#8b5cf6', '#0ea5e9', '#4f46e5', '#0284c7', '#818cf8', '#22d3ee', '#64748b', '#94a3b8'];
...
function MarketComparisonTip({ active, payload, label, model, period, mode = 'raw', metricKey }) {
  if (!active || !payload?.length) return null;
  const datum = payload[0]?.payload ?? {};
  const key = mode === 'singleGrowth' ? datum.key : metricKey ?? payload.find(item => METRICS.some(metric => metric.key === item.dataKey))?.dataKey;
  const metric = metrics.find(item => item.key === key);
  if (!metric) return <ComparisonTip active={active} payload={payload} label={label} growth={mode.includes('Growth')} monthly={mode === 'monthlyGrowth'} />;
  let rows = [];
  let title = label;
  let valuesAreGrowth = mode.includes('Growth');
  if (mode === 'raw' && datum.year != null) {
    const source = datum.year === period.year ? model.markets : model.comparisons.find(reference => reference.year === datum.year)?.markets ?? [];
    rows = source.map(market => ({ ...market, value: market[key] }));
    ...
    rows = model.marketSeries.map(market => {
      const current = market.series[index]?.[key];
      const baseline = market.series[index - 1]?.[key];
      return { ...market, value: current != null && baseline > 0 ? (current / baseline - 1) * 100 : null };
    });
    title = `${datum.label ?? label} vs previous month · ${metric.label}`;
  } else {
    const referenceYear = mode === 'annualGrowth' ? Number(label) : model.comparisons[0]?.year;
    const reference = model.comparisons.find(item => item.year === referenceYear);
    rows = model.markets.map(market => {
      const current = market[key];
      const baseline = reference?.markets.find(item => item.id === market.id)?.[key];
      return { ...market, value: current != null && baseline > 0 ? (current / baseline - 1) * 100 : null };
    });
    title = `${period.year} vs ${referenceYear ?? 'baseline'} · ${metric.label}`;
  }
  rows = rows.filter(market => market.value != null).sort((a, b) => b.value - a.value);
  const aggregate = payload.find(item => item.dataKey === key)?.value ?? payload[0]?.value;
...
export function YearComparison({ model, period }) {
  ...
  return <section className="ex-panel ex-year-comparison" id="year-comparison">
    ...
    <div className="ex-history-scroll" tabIndex={0} aria-label={`${monthly ? 'Monthly' : 'Historical'} values chart; scroll horizontally on small screens`}><div className="ex-history-chart" style={{ minWidth: Math.max(300, rows.length * 88 + 85) }}><ResponsiveContainer width="100%" height="100%"><BarChart data={rows} margin={{ top: 28, right: 20, bottom: 6, left: 10 }} accessibilityLayer><CartesianGrid vertical={false} stroke="#e5ebe7" strokeDasharray="3 5" /><XAxis dataKey="label" {...axis} /><YAxis {...axis} width={60} tickFormatter={value => compact(value)} /><Tooltip content={<MarketComparisonTip model={model} period={period} metricKey={metric.key} />} cursor={{ fill: '#f5f7f2' }} /><Bar dataKey={metric.key} name={metric.label} maxBarSize={76} radius={[5, 5, 0, 0]} isAnimationActive={false}>{rows.map(row => <Cell key={row.label} fill={row.fill} />)}<LabelList dataKey={metric.key} position="top" formatter={value => compact(value, 2)} fill="#52674f" fontSize={13} offset={9} /></Bar></BarChart></ResponsiveContainer></div></div>
