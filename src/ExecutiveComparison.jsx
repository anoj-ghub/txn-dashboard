...
const metrics = METRICS.map((metric, index) => ({ ...metric, color: ['#21876e', '#497ea8', '#c08734', '#9b72b0'][index] }));
...
function ComparisonTip({ active, payload, label, growth = false }) {
  if (!active || !payload?.length) return null;
  return <div className="ex-tooltip"><strong>{growth ? `Compared with ${label}` : label}</strong>{payload.filter(item => item.value != null).map(item => <div key={item.dataKey}><i style={{ background: item.color || item.payload.fill }} /><span>{item.name}</span><b>{growth ? percent(item.value) : number(Math.round(item.value))}</b></div>)}</div>;
}
...
export function ExecutiveReadout({ model, period }) {
  ...
  return <section className="ex-readout ex-readout-expanded" aria-label="Executive readout">
    <div className="ex-readout-heading"><span><Sparkles size={20} />THE READOUT</span><p>{period.range} compared with {all ? 'each earlier year' : comparisonLabel(model)} · four perspectives</p></div>
    <div className="ex-readout-points">{metrics.map((metric, index) => {
      ...
      const leaders = !all && model.comparisons.length ? model.markets.map(market => ({ ...market, change: metricComparisons(model, metric.key, market.id)[0]?.growth })).filter(market => market.change != null).sort((a, b) => b.change - a.change) : [];
...
export function YearComparison({ model, period }) {
  ...
  return <section className="ex-panel ex-year-comparison" id="year-comparison">
    ...
    <div className="ex-history-scroll" tabIndex={0} aria-label="Historical values chart; scroll horizontally on small screens"><div className="ex-history-chart" style={{ minWidth: Math.max(300, rows.length * 88 + 85) }}><ResponsiveContainer width="100%" height="100%"><BarChart data={rows} margin={{ top: 28, right: 20, bottom: 6, left: 10 }} accessibilityLayer><CartesianGrid vertical={false} stroke="#e5ebe7" strokeDasharray="3 5" /><XAxis dataKey="label" {...axis} /><YAxis {...axis} width={60} tickFormatter={value => compact(value)} /><Tooltip content={<ComparisonTip />} cursor={{ fill: '#f5f7f2' }} /><Bar dataKey={metric.key} name={metric.label} maxBarSize={76} radius={[5, 5, 0, 0]} isAnimationActive={false}>{rows.map(row => <Cell key={row.year} fill={row.fill} />)}<LabelList dataKey={metric.key} position="top" formatter={value => compact(value, 2)} fill="#52674f" fontSize={13} offset={9} /></Bar></BarChart></ResponsiveContainer></div></div>
    {rows.length > 3 && <p className="ex-scroll-hint">Swipe or scroll to see every year →</p>}
    <details className="ex-year-table"><summary>Show all four metrics by year <span>Values + change from each baseline</span></summary><p>Percentages show {period.year} compared with the year in that row. Unavailable or zero baselines produce no percentage; incomplete transaction periods are never summed.</p><div className="ex-table-scroll"><table><thead><tr><th>Year</th>{metrics.map(item => <th key={item.key}>{item.label}</th>)}</tr></thead><tbody>{years.map(row => <tr key={row.year} className={row.year === period.year ? 'reporting-year' : ''}><th>{row.year}{row.year === period.year ? ' · reporting' : ''}</th>{metrics.map(item => <td key={item.key}><strong title={number(row[item.key])}>{compact(row[item.key], 2)}</strong>{row.year !== period.year && <span>{percent(metricComparisons(model, item.key).find(value => value.year === row.year)?.growth ?? null)}</span>}</td>)}</tr>)}</tbody></table></div></details>
    <div className="ex-chart-footer">Each year remains separate. Cards and accounts are month-end balances. Unavailable values are shown as gaps.</div>
  </section>;
}
  ...
  return <section className="ex-panel ex-combined" id="combined"><div className="ex-panel-heading"><div><span className="ex-chart-eyebrow"><i />THE COMBINED VIEW</span><h2>Four indicators. One perspective.</h2><p>{indexed ? `${period.year} monthly movement · ${MONTHS[period.start - 1]} = 100` : all ? `${period.year} growth versus each earlier year · same markets and months` : `Like-for-like change against ${comparisonLabel(model)} · all selected markets`}</p></div><div className="ex-segments"><button aria-pressed={!indexed} className={!indexed ? 'active' : ''} onClick={() => setIndexed(false)}>Comparison growth</button><button aria-pressed={indexed} className={indexed ? 'active' : ''} onClick={() => setIndexed(true)}>Indexed trends</button></div></div>
      <div className="ex-combined-body"><div className="ex-combined-chart-area">
        ...
        {indexed ? <div className="ex-chart ex-index-chart"><ResponsiveContainer width="100%" height="100%"><LineChart data={indexedRows} margin={{ left: 5, right: 15, top: 15, bottom: 5 }} accessibilityLayer><CartesianGrid vertical={false} stroke="#e5ebe7" strokeDasharray="3 5" /><XAxis dataKey="label" {...axis} /><YAxis {...axis} width={45} domain={['auto', 'auto']} tickFormatter={value => Number(value).toFixed(0)} /><ReferenceLine y={100} stroke="#bccfc5" strokeDasharray="4 4" /><Tooltip formatter={value => `${Number(value).toFixed(1)} points`} />{metrics.filter(metric => visible.includes(metric.key)).map(metric => <Line key={metric.key} type="monotone" dataKey={metric.key} name={metric.label} stroke={metric.color} strokeWidth={3} dot={{ r: 3, stroke: '#fff', strokeWidth: 1 }} isAnimationActive={false} />)}</LineChart></ResponsiveContainer></div>
          : !hasGrowth ? <div className="ex-chart-empty">No complete, nonzero comparison baseline. Select another year or switch to indexed trends.</div>
          : all ? <div className="ex-chart ex-growth-chart"><ResponsiveContainer width="100%" height="100%"><BarChart data={yearlyGrowth} margin={{ top: 20, left: 4, right: 14, bottom: 5 }} accessibilityLayer><CartesianGrid vertical={false} stroke="#e5ebe7" strokeDasharray="3 5" /><XAxis dataKey="label" {...axis} /><YAxis {...axis} width={54} tickFormatter={value => `${value}%`} /><ReferenceLine y={0} stroke="#a8b9b0" /><Tooltip content={<ComparisonTip growth />} />{metrics.filter(metric => visible.includes(metric.key)).map(metric => <Bar key={metric.key} dataKey={metric.key} name={metric.label} fill={metric.color} maxBarSize={24} radius={[3, 3, 0, 0]} isAnimationActive={false} />)}</BarChart></ResponsiveContainer></div>
      ...
      </div><aside className="ex-combined-note"><span><Sparkles size={18} />READING THE SIGNAL</span><h3>{all ? 'Every year has its own baseline.' : best ? `${best.short} ${best.growth >= 0 ? 'lead growth' : 'show the smallest decline'}.` : 'Start with a shared baseline.'}</h3><p>{all ? <>Each group shows {period.year} relative to that year. Percentages are cumulative changes between the two years, not annualized rates or an average of history.</> : best ? <>At <strong>{percent(best.growth)}</strong> versus {comparisonLabel(model)}, this is the strongest relative change among the four indicators.</> : 'Comparison growth needs complete, nonzero baseline data for the same markets and months.'}</p><div><strong>Consistent comparisons</strong><p>Transactions use the period total. Cards and accounts use the last selected month. Indexed trends show the reporting year relative to its first selected month.</p></div></aside></div>
    </section>;
}
