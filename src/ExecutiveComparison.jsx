...
export function YearComparison({ model, period, breakdown, onBreakdownChange }) {
  ...
  return <section className="ex-panel ex-year-comparison" id="year-comparison">
    <div className="ex-panel-heading ex-breakdown-heading"><div><span className="ex-chart-eyebrow"><i />{monthly ? 'WITHIN THE YEAR' : 'MATCHED PERIODS, EVERY YEAR'}</span><h2>{monthly ? `${period.year}, month by month.` : model.mode === 'all' ? 'Compare all previous years.' : `Compare ${period.year} with ${comparisonLabel(model)}.`}</h2><p>{monthly ? `${period.selectedShort} ${period.year} · ${model.markets.length} selected markets · ${metric.kind === 'flow' ? 'monthly volumes' : 'month-end balances'}` : `${period.short} in every year · same ${model.markets.length} selected markets · ${metric.kind === 'flow' ? 'period totals' : model.hasReportedMonths ? `${MONTHS[period.end - 1]} snapshots` : 'no reported snapshot'}`}</p></div><div className="ex-segments ex-breakdown-toggle" role="group" aria-label="Time breakdown"><button aria-pressed={!monthly} className={!monthly ? 'active' : ''} onClick={() => onBreakdownChange('year')}>Year-wise comparison</button><button aria-pressed={monthly} className={monthly ? 'active' : ''} onClick={() => onBreakdownChange('month')}>Month-wise within year</button></div></div>
